...
function adrotate_renew($id, $howlong = 2592000) {
	...
	if($id > 0) {
		...
		if($schedule_id > 0) {
			$stopwhen = $wpdb->get_row($wpdb->prepare("SELECT `id`, `stoptime` FROM `".$wpdb->prefix."adrotate_schedule` WHERE `id` = %d ORDER BY `id` DESC LIMIT 1;", $schedule_id));
			$stoptime = $stopwhen->stoptime + $howlong;
			$wpdb->update($wpdb->prefix.'adrotate_schedule', array('stoptime' => $stoptime, 'maxclicks' => 0, 'maximpressions' => 0), array('ad' => $stopwhen->id));
		} else {
			$now = adrotate_now();
