...
/*-------------------------------------------------------------
 ...
 Since:		3.10.12
-------------------------------------------------------------*/
function adrotate_count_impression($ad, $group = 0, $type = '') { 
	...
	if(($adrotate_config['enable_loggedin_impressions'] == 'Y' AND is_user_logged_in()) OR !is_user_logged_in()) {
		...
		if($nocrawler == true AND strlen($useragent) > 0) {
			...
			adrotate_cookie_impressions($ad, $now, $type);
		}
	}
} 
...
function adrotate_impression_callback() {
	...
	if(is_numeric($ad) AND is_numeric($group)) {
		...
		if(($adrotate_config['enable_loggedin_clicks'] == 'Y' AND is_user_logged_in()) OR !is_user_logged_in()) {
			...
			if(!in_array(1, $nocrawler) AND !empty($useragent) AND $remote_ip != "unknown" AND !empty($remote_ip)) {
				...
				if($timer[1] < $click_timer) {
					...
					adrotate_cookie_clicks($ad, $now);
				}
			}
		}

		// Advertising budget
		$wpdb->query("UPDATE `".$prefix."adrotate` SET `cbudget` = `cbudget` - `crate` WHERE `id` = $ad AND `crate` > 0;");
