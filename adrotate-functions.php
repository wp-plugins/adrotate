...
function adrotate_shortcode($atts, $content = null) {
	...
	if($banner_id > 0 AND ($group_ids == 0 OR $group_ids > 0) AND $block_id == 0) $output .= adrotate_ad($banner_id); // Show one Ad
	if($banner_id == 0 AND $group_ids > 0 AND $block_id == 0) $output .= adrotate_group($group_ids, $fallback, $weight); // Show group
	if($banner_id == 0 AND $group_ids == 0 AND $block_id > 0) $output .= adrotate_block($block_id, $weight); // Show block
...
function adrotate_filter_schedule($selected, $banner) { 
	...
	foreach($schedules as $schedule) {
		if($adrotate_config['enable_stats'] == 'Y') {
			...
			if($adrotate_debug['general'] == true) {
				echo "<p><strong>[DEBUG][adrotate_filter_timeframe()] Schedule (id: ".$schedule->id.")</strong><pre>";
...
function adrotate_check_config() {
	...
	if(empty($config['jtools'])) $config['jtools'] = 'Y';
	if(empty($config['jshowoff'])) $config['jshowoff'] = 'Y';
