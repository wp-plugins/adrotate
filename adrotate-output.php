				...
				if($array_count > 0) {
					...
					if($group->modus == 1) { // Slider ads
						...
						foreach($selected as $key => $value) {
							$output .= '<div class="grid-dyn grid-col-1 a-'.$key.'">';
						...
						for($i=1;$i<=$block_count;$i++) {
							...
							$output .= '<div class="grid-col grid-col-'.$group->gridcolumns.' b-'.$group->id.' a-'.$banner_id.'">';
							if($wrapper_before != '') $output .= stripslashes(html_entity_decode($wrapper_before, ENT_QUOTES));
						...
						$banner_id = array_rand($selected);
						$output .= '<div class="grid-single grid-col-1 a-'.$banner_id.'">';
						if($wrapper_before != '') $output .= stripslashes(html_entity_decode($wrapper_before, ENT_QUOTES));
						$output .= adrotate_ad($banner_id, 0, $group->id);
						if($wrapper_after != '') $output .= stripslashes(html_entity_decode($wrapper_after, ENT_QUOTES));							
						$output .= '</div>';
...
function adrotate_custom_head() {
	...
	if($groups) {
		...
		if($array_css) {
			$output_css = "\t.g { positive:relative; clear:both; padding:0px; overflow:hidden; line-height:1; zoom:1; }\n";
			$output_css .= "\t.g:before, .g:after { content:\"\"; display:table; }\n";
			$output_css .= "\t.g:after { clear:both; }\n";
			$output_css .= "\t.grid-dyn { display:none; }\n";
			$output_css .= "\t.grid-col { position:relative; float:left; }\n";
			$output_css .= "\t.grid-col:first-child { margin-left: 0; }\n";
			$output_css .= "\t.grid-col-1 { width: 100%; }\n";
			$output_css .= "\t.grid-col-2 { width: 49.2%; }\n";
			$output_css .= "\t.grid-col-3 { width: 32.26%; }\n";
			$output_css .= "\t.grid-col-4 { width: 23.8%; }\n";
			$output_css .= "\t.grid-col-5 { width: 18.72%; }\n";
			$output_css .= "\t.grid-col-6 { width: 15.33%; }\n";
			$output_css .= "\t.grid-col-7 { width: 12.91%; }\n";
			$output_css .= "\t.grid-col-8 { width: 11.1%; }\n";
			$output_css .= "\t.grid-col-9 { width: 9.68%; }\n";
			$output_css .= "\t.grid-col-10 { width: 8.56%; }\n";
			$output_css .= "\t.grid-col-11 { width: 7.63%; }\n";
			$output_css .= "\t.grid-col-12 { width: 6.86%; }\n";			
			$output_css .= "\t@media only screen and (max-width: 480px) {\n";
			$output_css .= "\t\t.grid-col-2, .grid-col-3, .grid-col-4, .grid-col-5, .grid-col-6, .grid-col-7, .grid-col-8, .grid-col-9, .grid-col-10, .grid-col-11, .grid-col-12 { width: 100%; }\n";
			$output_css .= "\t}\n";
		...
		if(count($group_array) > 0) {
			...
			$ads = $wpdb->get_results(
				...
				FROM 
					...
					`".$prefix."adrotate_linkmeta` 
				WHERE 
					`".$prefix."adrotate_linkmeta`.`group` = '".$group_select."'
					AND `".$prefix."adrotate_linkmeta`.`block` = 0 
					AND `".$prefix."adrotate_linkmeta`.`user` = 0 
					AND `".$prefix."adrotate`.`id` = `".$prefix."adrotate_linkmeta`.`ad` 
					AND (`".$prefix."adrotate`.`type` = 'active' 
