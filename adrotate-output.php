...
function adrotate_ad($banner_id, $individual = true, $group = 0, $block = 0, $site = 0) {
	...
	if($banner_id) {
		...
		if($selected) {
			...
			if($banner->responsive == 'Y' AND $individual == true) {
				$responsive = ' a-responsive';
			} else {
				$responsive = '';
			}

			$output .= '<div class="a-single a-'.$banner->id.$responsive.'">';
	...
	if($group_ids) {
		...
		if($group) {
			...
			if($ads) {
				...
				foreach($ads as $ad) {
					$selected[$ad->id] = array('weight' => 6, 'responsive' => $ad->responsive);
				...
				if($array_count > 0) {
					...
					if($group->modus == 1) { // Slider ads
						...
						foreach($selected as $key => $value) {
							...
							if($value['responsive'] == 'Y') {
								$responsive = ' a-responsive';
							} else {
								$responsive = '';
							}

							$output .= '<div class="g-dyn a-'.$key.$responsive.'">';
						...
						for($i=1;$i<=$block_count;$i++) {
							...
							if($selected[$banner_id]['responsive'] == 'Y') {
								$responsive = ' a-responsive';
							} else {
								$responsive = '';
							}

							$output .= '<div class="g-col b-'.$group->id.' a-'.$banner_id.$responsive.'">';
						...
						$output .= '<div class="g-single a-'.$banner_id.$responsive.'">';
						$output .= $before.adrotate_ad($banner_id, 0, $group->id).$after;
						$output .= '</div>';
					}
...
function adrotate_inject_posts($post_content) { 
	...
	if(is_page() OR is_single()) {
		...
		$groups = $wpdb->get_results("SELECT `id`, `cat`, `cat_loc`, `cat_par`, `page`, `page_loc`, `page_par`, `wrapper_before`, `wrapper_after` FROM `".$prefix."adrotate_groups` WHERE `page_loc` > 0 OR `cat_loc` > 0 ORDER BY `id` ASC;");
		...
		if(count($group_array) > 0) {
			...
			if($ads) {
				...
				if($group_array[$group_select]['location'] == 1 OR $group_array[$group_select]['location'] == 3) {
					...
					$post_content = '<div class="g g-'.$group_array[$group_select]['group'].'"><div class="g-postinject a-'.$banner_id.$responsive.'">'.$before.adrotate_ad($banner_id, 0, $group_array[$group_select]['group']).$after.'</div></div>'.$post_content;
					$selected = array_diff_key($selected, array($banner_id => 0));
				}
				...
				if($group_array[$group_select]['location'] == 2 OR $group_array[$group_select]['location'] == 3) {
					...
					$post_content = $post_content.'<div class="g g-'.$group_array[$group_select]['group'].'"><div class="g-postinject a-'.$banner_id.$responsive.'">'.$before.adrotate_ad($banner_id, 0, $group_array[$group_select]['group']).$after.'</div></div>';
					$selected = array_diff_key($selected, array($banner_id => 0));
				}
				...
				if($group_array[$group_select]['location'] == 4) {
					...
					$par = 1;
					$post_content = '';
					$allads = count($selected);
					$allparagraphs = count($paragraphs);
					foreach($paragraphs as $paragraph) {
						if($par == $group_array[$group_select]['paragraph'] 
						  ...
						  OR ($par == 4 AND $group_array[$group_select]['paragraph'] == 40)) {
							...
							$paragraph = $paragraph.'<div class="g g-'.$group_array[$group_select]['group'].'"><div class="g-postinject a-'.$banner_id.$responsive.'">'.$before.adrotate_ad($banner_id, 0, $group_array[$group_select]['group']).$after.'</div></div>';

							if($allads >= $allparagraphs) {
								$selected = array_diff_key($selected, array($banner_id => 0));
							}
...
function adrotate_custom_head() {
	...
	if($groups) {
		...
		if($array_css) {
			...
			$output_css .= "\t\t.g-col, .g-dyn, .g-single, .g-postinject { width:100%; margin-left:0; margin-right:0; }\n";
			$output_css .= "\t}\n";
			unset($array_css);
		}
	}
...
function adrotate_credits() {
	...
	echo __('Buy AdRotate Professional here', 'adrotate').' <a href="https://www.adrotateplugin.com/adrotate-pro/" target="_blank">adrotateplugin.com shop</a>.<br />';
	echo __('Check out the', 'adrotate').' <a href="https://www.adrotateplugin.com/support/knowledgebase/" target="_blank">'.__('manuals', 'adrotate').'</a> '.__('and have the most popular features explained.', 'adrotate').'<br />';
