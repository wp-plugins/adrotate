...
function adrotate_ad_output($id, $group = 0, $block = 0, $bannercode, $tracker, $link, $image) {
	...
	if($tracker == "Y" AND $adrotate_config['enable_stats'] == 'Y') {
		if($adrotate_debug['track'] == true) {
			$meta = "$id,$group,$block,0";
		} else {
			$meta = base64_encode("$id,$group,$block,0");
		}

		$banner_output = str_replace('%link%', $link, $banner_output);
		if(preg_match('/<a[^>]+class=\"(.+?)\"[^>]*>/i', $banner_output, $regs)) {
		    $result = explode(" ", $regs[1]);
		    $result[] = "gofollow";
		    $result = implode(" ", $result);
			$banner_output = preg_replace('/class=\"(.+?)\"/i', 'class="'.$result.'"', $banner_output);	    
		} else {
...
function adrotate_custom_scripts() {
	...
	wp_enqueue_script('clicktracker', plugins_url('/library/jquery.clicktracker.js', __FILE__), array('jquery'), '0.1');
}
