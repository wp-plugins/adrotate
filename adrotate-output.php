...
function adrotate_inject_pages($post_content) { 
	...
	if(is_page()) {
		...
		if($adrotate_debug['general'] == true) {
			echo "<p><strong>[DEBUG][adrotate_inject_posts()] page_array</strong><pre>"; 
			print_r($page_array); 
			echo "</pre></p>"; 
		}			
		unset($ids, $pages);
		...
		if(count($page_array) > 0) {
			if(count($page_array) > 1) {
				...
				if($choice_amount == 0 OR $page_count < $choice_amount) {
					$choice_amount = $page_count;
				}
			...
			if($adrotate_debug['general'] == true) {
				...
				echo "Choice Amount (Paragraphs): ".$choice_amount;
				echo "<br />Page Array (Groups): ".count($page_array)."</br>";
				print_r($page_choice); 
				echo "</pre></p>"; 
			}

			foreach($page_choice as $key => $group_id) {
				if(is_page($page_array[$group_id]['pages'])) {
					// Advert in front of content
					if($page_array[$group_id]['location'] == 1 OR $page_array[$group_id]['location'] == 3) {
				   		$post_content = adrotate_group($page_array[$group_id]['group']).$post_content;
					}
		
					// Advert behind the content
					if($page_array[$group_id]['location'] == 2 OR $page_array[$group_id]['location'] == 3) {
				   		$post_content = $post_content.adrotate_group($page_array[$group_id]['group']);
					}
	
					// Adverts inside the content
					if($page_array[$group_id]['location'] == 4) {
						$paragraphs = explode("</p>", $post_content);
						$par = 1;
						$post_content = '';
						foreach($paragraphs as $paragraph) {
							if($par == $page_array[$group_id]['paragraph']
							  OR ($par == 2 AND $page_array[$group_id]['paragraph'] == 20) 
							  OR ($par == 3 AND $page_array[$group_id]['paragraph'] == 30) 
							  OR ($par == 4 AND $page_array[$group_id]['paragraph'] == 40)) {
								$paragraph = $paragraph.adrotate_group($page_array[$group_id]['group']);
	
								if($page_array[$group_id]['paragraph'] > 1 AND $page_array[$group_id]['paragraph'] < 10) {
									$par = 1;
								} else {
									$par++;
								}
		...
		unset($page_choice, $page_count, $page_array, $paragraph);
	}
	return $post_content;
}
...
function adrotate_inject_posts($post_content) { 
	...
	if(is_single()) {
		...
		if($adrotate_debug['general'] == true) {
			echo "<p><strong>[DEBUG][adrotate_inject_posts()] cat_array</strong><pre>"; 
			print_r($cat_array); 
			echo "</pre></p>"; 
		}			
		unset($ids, $cats);
		...
		if(count($cat_array) > 0) {
			if(count($cat_array) > 1) {
				...
				if($choice_amount == 0 OR $cat_count < $choice_amount) {
					$choice_amount = $cat_count;
				}
			...
			if($adrotate_debug['general'] == true) {
				echo "<p><strong>[DEBUG][adrotate_inject_posts()] cat_choice</strong><pre>"; 
				print_r($cat_choice); 
				echo "</pre></p>"; 
			}
	
			foreach($cat_choice as $key => $group_id) {
				if(in_category($cat_array[$group_id]['categories'])) {
					// Advert in front of content
					if($cat_array[$group_id]['location'] == 1 OR $cat_array[$group_id]['location'] == 3) {
						$post_content = adrotate_group($cat_array[$group_id]['group']).$post_content;
					}
					
					// Advert behind content
					if($cat_array[$group_id]['location'] == 2 OR $cat_array[$group_id]['location'] == 3) {
				   		$post_content = $post_content.adrotate_group($cat_array[$group_id]['group']);
					}
	
					// Adverts inside the content
					if($cat_array[$group_id]['location'] == 4) {
						$paragraphs = explode("</p>", $post_content);
						$par = 1;
						$post_content = '';
						foreach($paragraphs as $paragraph) {
							if(($par == $cat_array[$group_id]['paragraph']
							  OR ($par == 2 AND $cat_array[$group_id]['paragraph'] == 20) 
							  OR ($par == 3 AND $cat_array[$group_id]['paragraph'] == 30) 
							  OR ($par == 4 AND $cat_array[$group_id]['paragraph'] == 40))) {
								$paragraph = $paragraph.adrotate_group($cat_array[$group_id]['group']);
	
								if($cat_array[$group_id]['paragraph'] > 1 AND $cat_array[$group_id]['paragraph'] < 10) {
									$par = 1;
								} else {
									$par++;
								}
		...
		unset($cat_choice, $cat_count, $cat_array, $paragraph);
	}
