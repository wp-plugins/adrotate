	...
	if($groups) {
		...
		foreach($groups as $group) {
			...
			if($group->adwidth != 'auto') {
				$adwidth = ($group->admargin*2) + $group->adwidth.'px'; 
				if($group->modus == 2) $gridwidth = $group->gridcolumns*(($group->admargin*2) + $group->adwidth).'px';
			}
			if($group->adheight != 'auto') {
				$adheight = ($group->admargin*2) + $group->adheight.'px'; 
				if($group->modus == 2) $gridheight = $group->gridrows*(($group->admargin*2) + $group->adheight).'px';
			}
			...
			if($group->modus == 1) {
				...
				$array_css[] = "\t.g-".$group->id." { min-height:".$group->adheight."; max-height:".$adheight."; min-width:".$group->adwidth."; max-width:".$adwidth."; }\n";
			}
