			...
			if($ads) {
				...
				if($array_count > 0) {
					...
					if($group->modus == 1) { // Slider ads
						...
						$output .= '<div id="g-'.$group->id.'"><ul id="gs-'.$group->id.'" class="gs">';
						foreach($selected as $key => $value) {
							$output .= '<li class="a-'.$value['id'].' g-'.$value['group'].' gs">';
						...
						$output .= '<div id="b-'.$group->id.'" class="block_outer b-'.$group->id.'"><ul class="b">';
						for($i=0;$i<$block_count;$i++) {
							$banner_id = array_rand($selected, 1);
							$output .= '<li id="b b-'.$group->id.' a-'.$banner_id.' g-'.$selected[$banner_id]['group'].'" class="block_inner">';
							if($wrapper_before != '') $output .= stripslashes(html_entity_decode($wrapper_before, ENT_QUOTES));
	...
	if($groups) {
		...
		foreach($groups as $group) {
			if($group->adwidth == 'auto') {
				...
				$gridwidth = $group->gridcolumns*((2*$group->admargin)+$group->adwidth).'px';
			}
			if($group->adheight == 'auto') {
				$adheight = $gridheight = 'auto';
			} else {
				$adheight = $group->adheight.'px';
				$gridheight = $group->gridrows*((2*$group->admargin)+$group->adheight).'px';
			}
			if($group->modus == 1) {
				$array_js[] = "\t$('#gs-".$group->id."').jshowoff({ links: false, controls: false, effect: 'fade', changeSpeed: 300, speed: ".$group->adspeed.", hoverPause: true });\n";
				$array_css[] = "\t#g-".$group->id." { position:relative; overflow:hidden; width:".$adwidth."; height:".$adheight."; }\n";
				$array_css[] = "\t#gs-".$group->id." ul, #gs-".$group->id." li { margin:".$group->admargin."px; }\n";
			}
			if($group->modus == 2) {
				$array_css[] = "\t.b-".$group->id." { min-height:".$adheight."; max-height:".$gridheight."; min-width:".$adwidth."; max-width:".$gridwidth."; }\n";
				$array_css[] = "\t.b-".$group->id." li { margin:".$group->admargin."px; float:left; height:".$adheight."; width:".$adwidth."; }\n";
			}
		...
		if($array_css) {
			...
			$output .= "\t.b { padding:0; margin:0; overflow:hidden; }\n";
			$output .= "\t.b ul { margin:0 auto; width:100%; height:100%; text-align:center; }\n";
			$output .= "\t.gs ul, .gs li, .b ul, .b li { padding:0; list-style:none; }";
