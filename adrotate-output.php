			...
			if($ads) {
				...
				if($array_count > 0) {
					...
					if($group->modus == 1) { // Slider ads
						...
						$output .= '<div id="a-'.$group->id.'" class="g g-'.$group->id.'">';
						$i = 1;
						foreach($selected as $key => $value) {
							$output .= '<div id="a-'.$key.'" class="a-'.$group->id.' c-'.$i.'">';
...
function adrotate_custom_scripts() {
	...
	wp_enqueue_script('clicktracker', plugins_url('/library/jquery.clicktracker.js', __FILE__), false, '0.6', $in_footer);
}
