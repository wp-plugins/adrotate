	...
	if($groups) {
		...
		foreach($groups as $group) {
			...
			if($group->adwidth != 'auto') {
				...
				$adwidth = $width_sum.'px'; 
				if($group->modus == 1) {
					$dynamic_width = "min-width:".$group->adwidth."px; max-width:".$width_sum."px;";
				}
				if($group->modus == 2) {
 					$grid_width = "min-width:".$adwidth."px; max-width:".$group->gridcolumns * $width_sum + $safety."px;";
				}
			} else {
			...
			if($group->adheight != 'auto') {
				...
				$adheight = $height_sum.'px'; 
				if($group->modus == 1) {
					$dynamic_height = "min-height:".$group->adheight."px; max-height:".$height_sum."px;";
				}
				if($group->modus == 2) {
					$grid_height = "min-height:".$adheight."px; max-height:".$group->gridrows * $height_sum + $safety."px;";
				}
			} else {
