...
function adrotate_custom_scripts() {
	...
	wp_enqueue_script('jquerytools-min', plugins_url('/library/jquery.tools.min.js', __FILE__));
	wp_enqueue_script('jshowoff-min', plugins_url('/library/jquery.jshowoff.min.js', __FILE__));
}
...
function adrotate_custom_head() {
	...
	$groups = $wpdb->get_results("SELECT `id`, `modus`, `adwidth`, `adheight`, `admargin`, `adspeed` FROM `".$wpdb->prefix."adrotate_groups` WHERE `modus` > 0 ORDER BY `id` ASC;");
	...
	if($groups) {
		...
		foreach($groups as $group) {
			...
			if($group->modus == 2) {
				$gridwidth = 2*((2*$group->admargin)+$group->adwidth).'px';
				$gridheight = 2*((2*$group->admargin)+$group->adwidth).'px';
