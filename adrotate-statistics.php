...
function adrotate_stats($ad, $when = 0, $until = 0) {
	...
	if($when > 0 AND is_numeric($when) AND $until > 0 AND is_numeric($until)) $whenquery =  " AND `thetime` >= '$when' AND `thetime` <= '$until' GROUP BY `ad` ASC";
	else if($when > 0 AND is_numeric($when) AND $until == 0) $whenquery =  " AND `thetime` = '$when'";
	else $whenquery = "";

	$stats = $wpdb->get_row("SELECT SUM(`clicks`) as `clicks`, SUM(`impressions`) as `impressions` FROM `".$wpdb->prefix."adrotate_stats` WHERE `ad` = '$ad'$whenquery;", ARRAY_A);
