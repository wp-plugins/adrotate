...
$ads = $wpdb->get_results("SELECT `id`, `title`, `tracker`, `weight`, `crate`, `cbudget`, `irate`, `ibudget` FROM `".$wpdb->prefix."adrotate` WHERE (`type` != 'empty' AND `type` != 'a_empty') ORDER BY `id` ASC;");
	...
	<h3><?php _e('Advanced', 'adrotate'); ?></h3>
   	<table class="widefat" style="margin-top: .5em">
	    ...
	    <tr>
			...
			<td>
				<label for="adrotate_admargin"><input tabindex="9" name="adrotate_admargin" type="text" class="search-input" size="3" value="<?php echo $edit_group->admargin; ?>" autocomplete="off" /> <?php _e('pixel(s)', 'adrotate'); ?>.</label>
				</td>
			<td colspan="2">
	    ...
	    <tr>
			...
			<td width="35%"><label for="adrotate_geo"><input tabindex="10" type="checkbox" name="adrotate_geo" value="1" disabled /> <?php _e('Enable GeoTargeting for this group.', 'adrotate'); ?></label></td>
	    ...
	    <tr>
			...
			<td>
				<label for="adrotate_fallback"><select tabindex="11" name="adrotate_fallback" disabled>
   	...
   	<table class="widefat" style="margin-top: .5em">
      	<tr>
	        ...
	        <label for="adrotate_cat_paragraph">
		        <select tabindex="17" name="adrotate_cat_paragraph">
      	...
      	<tr>
	        ...
	        <label for="adrotate_page_paragraph">
		        <select tabindex="19" name="adrotate_page_paragraph">
   	...
   	<table class="widefat" style="margin-top: .5em">
		...
		<?php if($ads) {
			...
			foreach($ads as $ad) {
				...
				if(($ad->crate > 0 AND $ad->cbudget < 1) OR ($ad->irate > 0 AND $ad->ibudget < 1)) $errorclass = ' row_inactive';
