   	...
   	<table class="widefat" style="margin-top: .5em">
		...
		if ($blocks) {
			...
			foreach($blocks as $block) {
				$stats = $wpdb->get_row("SELECT SUM(`clicks`) as `clicks`, SUM(`impressions`) as `impressions` FROM `".$wpdb->prefix."adrotate_stats` WHERE `block` = '$block->id';", ARRAY_A);
				$stats_today = $wpdb->get_row("SELECT SUM(`clicks`) as `clicks`, SUM(`impressions`) as `impressions` FROM `".$wpdb->prefix."adrotate_stats` WHERE `block` = '$block->id' AND `thetime` = '$today';", ARRAY_A);
