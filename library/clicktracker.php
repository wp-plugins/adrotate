...
if(isset($_GET['track']) OR $_GET['track'] != '') {
	...
	list($ad, $group, $block, $remote) = explode(",", $meta, 4);
	...
	if(($adrotate_config['enable_loggedin_clicks'] == 'Y' AND is_user_logged_in()) OR !is_user_logged_in()) {
		...
		if($ip < 1 AND !in_array(1, $nocrawler) AND !empty($useragent)) {
			...
			if($remote_ip != "unknown" AND !empty($remote_ip)) {
				$wpdb->insert($prefix.'adrotate_tracker', array('ipaddress' => $remote_ip, 'timer' => $now, 'bannerid' => $ad, 'stat' => 'c', 'useragent' => $useragent, 'country' => $adrotate_geo['country'], 'city' => $adrotate_geo['city']));
			}
		}
	}

	if($remote === 1) {
		$bannerurl = $wpdb->get_row("SELECT `link` FROM `".$wpdb->prefix."adrotate` WHERE `id` = $ad;");
